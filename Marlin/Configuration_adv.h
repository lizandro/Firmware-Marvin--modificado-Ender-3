...
// @section bltouch
  ...
  /**
   *Perigo: não ative o modo 5V, a menos que seja anexado a um controlador tolerante a 5V!
   *V3.0 ou 3.1: Defina o modo padrão no modo 5V na inicialização do Marlin.
   *Se desativado, o modo OD é o padrão codificado no 3.0
   *Na startup, o Marlin comparará sua EEPROM com esse valor. Se o modo selecionado
   *Difere, um modo de modo que a gravação EEPROM será concluído na inicialização.
   *Use a opção abaixo para forçar uma gravação da EEPROM em uma sonda v3.1, independentemente.
   */
  //#Define BLTOUCH_SET_5V_MODE

  /**
   *Segurança: Ative se conectar uma sonda com um modo de tensão desconhecido.
   *V3.0: Defina uma sonda no modo selecionado acima na inicialização do Marlin. Necessário para o modo 5V no 3.0
   *V3.1: Força uma sonda com modo desconhecido no modo selecionado na startup marlin (= sonda eeprom write)
   *Para preservar a vida útil da sonda, use-a uma vez, desligue-a e re-flash.
   */
  //#Define BLTOUCH_FORCE_MODE_SET

  /**
   *Ativar opção de "alta velocidade" para sondagem.
   *Perigo: Desative se sua sonda às vezes falhar. Somente adequado para sistemas bem ajustados estáveis.
   *Esse recurso foi projetado para Deltabots com movimentos Z muito rápidos; No entanto, cartesianos de velocidade mais alta
   *pode ser capaz de usá -lo. Se a máquina não puder aumentar o Z rápido o suficiente, o Bltoch poderá entrar em alarme.
   *
   *Defina o estado padrão aqui, altere com 'M401 S' ou UI, use M500 para salvar, M502 para redefinir.
   */
...
/**
 *Calibração automática de reação, posição e offset HOTEND
 *
 *Ativar G425 para executar a calibração automática usando um eletricamente-
 *Cubo condutor, parafuso ou arruela montada na cama.
 *
 *G425 usa a sonda para tocar a parte superior e lados do objeto de calibração
 *Na cama e medidas e/ou compensações posicionais corretas, reação do eixo
 *e compensações gostosas.
 *
 *Nota: HOTEND_OFFSET e CALIBRATION_OBJECT_CENTER devem ser definidos como dentro
 *± 5mm de valores verdadeiros para G425 para ter sucesso.
 */
...
/**
 *A suavização de etapas adaptativas aumenta a resolução de movimentos de vários eixos, principalmente nas frequências de etapas
 *Abaixo de 1kHz (para AVR) ou 10kHz (para braço), onde o alias entre os eixos em movimentos de vários eixos causa audível
 *Vibração e artefatos de superfície. O algoritmo se adapta para fornecer a melhor etapa possível de suavização no
 *Frequências de passo mais baixas.
 */
...
//#define LCD_BACKLIGHT_TIMEOUT 30 // (s) Timeout before turning off the backlight
  ...
  #define PROBE_OFFSET_WIZARD       // Add a Probe Z Offset calibration option to the LCD menu
    ...
    /**
     *Ative para iniciar o deslocamento da sonda Z ao iniciar o assistente.
     *Use uma altura ligeiramente acima do deslocamento Z estimado do bico-sonda.
     *Por exemplo, com um deslocamento de -5, considere uma altura inicial de -4.
     */
  ...
  //#define LCD_TIMEOUT_TO_STATUS 15000   // (ms)
    ...
    #define BOOTSCREEN_TIMEOUT 4000       // (ms) Total Duration to display the boot screen(s)
      ...
      #define BOOT_MARLIN_LOGO_SMALL      // Mostre um logotipo de marlin menor na tela de inicialização (economizando muito flash)
    #endif
  #endif
...
/**
 *Babystepping permite o movimento dos eixos por pequenos incrementos sem mudar
 *Os valores atuais de posição. Este recurso é usado principalmente para ajustar o z
 *Eixo na primeira camada de uma impressão em tempo real.
 *
 *AVISO: não respeita os pontados de final!
 */
#define BABYSTEPPING
  ...
  #define BABYSTEP_ZPROBE_OFFSET          // Combine M851 Z and Babystepping
    ...
    //#Define babystep_hotend_z_offset //Para vários fontes, compensações de Z Babystep Z
 ...
 *Margens de sondagem
 *
 *Substitua sonding_margin para cada lado da placa de construção
 *Útil para obter pontos de sonda para posições exatas em alvos ou
 *Para permitir que o nivelamento evite grampos de placas apenas específicos
 *lados da cama. Com bico_as_probe valores negativos são
 *permitido, permitir a sondagem do lado de fora da cama.
 *
 *Se você estiver substituindo as opções anteriores *_probe_bed_Position,
 *Os valores esquerdo e frontal na maioria dos casos mapearão diretamente
 *Direito e traseiro seriam o inverso, como
 *(X/y_bed_size -right/back_probe_bed_position)
 *
 *Isso permitirá que todas as posições correspondam à compilação, no entanto
 *Caso a posição da sonda seja modificada com m851xy, então o
 *Os pontos da sonda seguirão. Isso impede que qualquer mudança cause
 *A sonda a ser incapaz de atingir nenhum ponto.
 */
